...
* Bryus Ekkel \(_Bruce Eckel\) \_mashxur_ \_ 'Thinking in Java' \("Javada fikrlash"\) va 'Thinking in C++' \("C++ da fikrlash"\) kitoblarining muallifi. Hech bir tili meni Python'chalik samarador qilmagan deydi u.  He says that Python is perhaps the only language that focuses on making things easier for the programmer. Read the [complete interview](http://www.artima.com/intv/aboutme.html) for more details.
